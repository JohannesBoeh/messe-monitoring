...
def get_detail_url(fair):
    # Confirmed URL pattern from live site:
    # https://www.auma.de/messen-finden/details/?tfd=berlin_grune-woche_223194
    url_param = fair.get("strUrlParameter", "")
    if url_param:
        return "{}/messen-finden/details/?tfd={}".format(AUMA_BASE, url_param)
    return ""
...
def scrape_with_playwright(url, debug=False):
    ...
    try:
        with sync_playwright() as pw:
            ...
            # The statistics section has collapsible sub-rows (e.g. "Besucher").
            # Click every toggle button/row inside the Statistiken section to expand them.
            page.evaluate("""
                // Expand Bootstrap-style collapses
                document.querySelectorAll('.collapse').forEach(el => el.classList.add('show'));

                // Click any expandable row headers inside the statistics table
                document.querySelectorAll(
                  '.trade-fair-statistics-table__row--lvl-1, ' +
            ...
            # Second pass: expand anything that appeared after clicks
            page.evaluate("""
                document.querySelectorAll('.collapse').forEach(el => el.classList.add('show'));
            """)
            time.sleep(1)
